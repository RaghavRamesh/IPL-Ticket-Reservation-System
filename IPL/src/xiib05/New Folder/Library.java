...
class Library
{
   JFrame jf;
	...
	JButton jb_insert,jb_find,jb_details,jb_remove;
	Connection con;
	ResultSet rs;
			   ...
			   if(e.getSource()==jb_insert)
				...
				if(e.getSource()==jb_remove)
				{
				   String bookid = jt_bookid.getText();
					try{
					   Statement S = con.createStatement();
						int r = S.executeUpdate("DELETE FROM LIBRARY WHERE BOOKID = "+bookid);
						if(r==1)
						{
						   JOptionPane.showMessageDialog(null,"Book deleted.");
							...
							jt_category.setText("");
						  	...
						  	JOptionPane.showMessageDialog(null,"No book with the given id was found.");
					}		
				   catch(Exception er)
					{
					   JOptionPane.showMessageDialog(null,er.getMessage());
